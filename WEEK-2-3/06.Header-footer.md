...
For example, we will see later that we can assign CSS files to the pages in the `functions.php` file. The `wp_head()` function will make sure to echo the link to this css file.
...
Again... `wp_foot()` inserts automatically all the scripts that are necessary to WordPress as well as any script you decided to insert in the `functions.php`
